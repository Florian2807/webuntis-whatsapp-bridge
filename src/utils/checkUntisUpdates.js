module.exports = async ({ classID }) => {
	...
	const existingChanges = wb.changedLessons[classID].filter(lesson => wb.Utils.parseUntisDate(lesson.date) / 1000 >= firstDayOfWeek / 1000);
	const newChanges = [];
	...
	Object.keys(timetable).forEach(day => {
		...
		lessonsPerDay[day] = dayLessons
			.map(lesson => wb.Utils.translateCellstate(lesson.cellState, lesson.teacher.filter(Boolean)).cellstate)
